...
class DoubleMLPanelData(DoubleMLData):
    ...
    def id_col(self, value):
        ...
        if value is not None:
            if not isinstance(value, str):
                raise TypeError('The time variable id_col must be of str type (or None). '
                                f'{str(value)} of type {str(type(value))} was passed.')
            if value not in self.all_variables:
                raise ValueError('Invalid time variable id_col. '
                                    f'{value} is no data column.')
            self._id_col = value
        else:
            self._id_col = None
    ...
    def _check_disjoint_sets_id_col(self):
        ...
        if not y_col_set.isdisjoint(id_col_set):
            raise ValueError(f'{str(self.y_col)} cannot be set as outcome variable ``y_col`` and id '
                             'variable in ``id_col``.')
        ...
        if not x_cols_set.isdisjoint(id_col_set):
            raise ValueError('At least one variable/column is set as covariate (``x_cols``) and id '
                             'variable in ``id_col``.')
        if self.z_cols is not None:
            z_cols_set = set(self.z_cols)
