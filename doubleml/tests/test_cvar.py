...
def test_doubleml_cvar_exceptions():
    ...
    with pytest.raises(ValueError, match=msg):
        ...
        _ = dml.DoubleMLCVAR(obj_dml_data_iv, ml_g, ml_m, treatment=1)
    ...
    with pytest.raises(ValueError, match=msg):
        ...
        _ = dml.DoubleMLCVAR(obj_dml_data_cont_treat, ml_g, ml_m, treatment=1)

    msg = 'Invalid trimming_rule cap. Valid trimming_rule truncate.'
    with pytest.raises(ValueError, match=msg):
        _ = dml.DoubleMLCVAR(obj_dml_data, ml_g, ml_m, treatment=1, trimming_rule="cap")

    msg = "trimming_threshold has to be a float. Object of type <class 'int'> passed."
    with pytest.raises(TypeError, match=msg):
        _ = dml.DoubleMLCVAR(obj_dml_data, ml_g, ml_m, treatment=1, trimming_threshold=-1)

    msg = 'Invalid trimming_threshold -0.4. trimming_threshold has to be between 0 and 0.5.'
    with pytest.raises(ValueError, match=msg):
        _ = dml.DoubleMLCVAR(obj_dml_data, ml_g, ml_m, treatment=1, trimming_threshold=-.4)

    msg = 'Invalid score cvar. Valid score CVaR.'
    with pytest.raises(ValueError, match=msg):
        _ = dml.DoubleMLCVAR(obj_dml_data, ml_g, ml_m, treatment=1, score="cvar")
    msg = 'Invalid score. Valid score CVaR.'
    with pytest.raises(TypeError, match=msg):
        _ = dml.DoubleMLCVAR(obj_dml_data, ml_g, ml_m, treatment=1, score=1)
