...
from ._utils_did_rcs_manual import fit_did_rcs, boot_did_rcs
...
@pytest.fixture(scope='module')
def dml_did_rcs_fixture(generate_data_did_rcs, learner, dml_procedure, trimming_threshold):
    boot_methods = ['normal']
    n_folds = 2
    n_rep_boot = 499

    # collect data
    (x, y, t, d) = generate_data_did_rcs
    ...
    np.random.seed(3141)
    obj_dml_data = dml.DiffInDiffRCDoubleMLData.from_arrays(x, y, d, t)
    dml_did_obj = dml.DoubleMLDiD(obj_dml_data,
                                  ml_g, ml_m,
                                  n_folds,
                                  score="ortho_rcs",
    ...
    res_manual = fit_did_rcs(y, x, d, t,
                             clone(learner[0]), clone(learner[1]),
                             all_smpls, dml_procedure, trimming_threshold=trimming_threshold)
    ...
    for bootstrap in boot_methods:
        np.random.seed(3141)
        boot_theta, boot_t_stat = boot_did_rcs(y, d, t, res_manual['thetas'], res_manual['ses'],
                                               res_manual['all_g_hat'], res_manual['all_m_hat'], res_manual['all_p_hat'],
                                               res_manual['all_lambda_hat'], all_smpls, bootstrap, n_rep_boot)
...
@pytest.mark.ci
def test_dml_did_ro_coef(dml_did_ro_fixture):
...
@pytest.mark.ci
def test_dml_did_rcs_coef(dml_did_rcs_fixture):
    assert math.isclose(dml_did_rcs_fixture['coef'],
                        dml_did_rcs_fixture['coef_manual'],
                        rel_tol=1e-9, abs_tol=1e-4)
...
@pytest.mark.ci
def test_dml_did_rcs_se(dml_did_rcs_fixture):
    assert math.isclose(dml_did_rcs_fixture['se'],
                        dml_did_rcs_fixture['se_manual'],
                        rel_tol=1e-9, abs_tol=1e-4)
...
@pytest.mark.ci
def test_dml_did_rcs_boot(dml_did_rcs_fixture):
    for bootstrap in dml_did_rcs_fixture['boot_methods']:
        assert np.allclose(dml_did_rcs_fixture['boot_coef' + bootstrap],
                           dml_did_rcs_fixture['boot_coef' +
                                               bootstrap + '_manual'],
                           rtol=1e-9, atol=1e-4)
        assert np.allclose(dml_did_rcs_fixture['boot_t_stat' + bootstrap],
                           dml_did_rcs_fixture['boot_t_stat' +
                                               bootstrap + '_manual'],
                           rtol=1e-9, atol=1e-4)
