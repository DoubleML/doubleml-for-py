...
from doubleml import DoubleMLPLR, DoubleMLIRM, DoubleMLIIVM, DoubleMLPLIV, DoubleMLDiD
...
def test_did_defaults():
    _assert_resampling_default_settings(dml_did_ro)
    assert dml_did_ro.n_folds == 5
    assert dml_did_ro.n_rep == 1
    assert dml_did_ro.score == 'ortho_ro'
    assert dml_did_ro.dml_procedure == 'dml2'
    assert dml_did_ro.trimming_rule == 'truncate'
    assert dml_did_ro.trimming_threshold == 1e-12
