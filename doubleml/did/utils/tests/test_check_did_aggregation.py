...
def test_valid_aggregation_dict(sample_gt_index, valid_weight_masks):
    ...
    valid_dict = {
        "weight_masks": valid_weight_masks,
        "agg_names": ["g1", "g2"],
...
def test_invalid_dimensions(sample_gt_index):
    ...
    wrong_dims = np.ma.array(
        np.zeros((sample_gt_index.shape)),  # Only 3 dimensions
        mask=sample_gt_index.mask
    )
...
def test_invalid_shape(sample_gt_index):
    ...
    wrong_shape = np.ma.array(
        np.zeros((3, 3, 3, 2)),  # Wrong shape for first 3 dimensions
        mask=np.zeros((3, 3, 3, 2), dtype=bool)
    )
...
def test_invalid_mask_alignment(sample_gt_index):
    ...
    weight_masks = np.ma.array(
        np.zeros((*sample_gt_index.shape, 2)),
        mask=np.broadcast_to(wrong_mask[..., np.newaxis], (*sample_gt_index.shape, 2))
    )
