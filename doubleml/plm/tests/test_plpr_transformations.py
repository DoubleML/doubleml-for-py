...
df_unbalance_float = df_unbalance_int.copy()
df_unbalance_float["time"] = df_unbalance_float["time"].astype(float)

df_unbalance_datetime = df_unbalance_int.copy()
df_unbalance_datetime["time"] = pd.to_datetime([f"2020-{int(t):02d}-01" for t in df_unbalance_datetime["time"]])
...
@pytest.fixture(scope="module", params=[df_unbalance_int, df_unbalance_float, df_unbalance_datetime])
def data_unbalance_time_type(request):
    return request.param
...
@pytest.mark.ci
def test_plpr_fd_exact_unbalanced(data_unbalance_time_type):
    msg_warn = r"The panel data contains 1 missing \(id, time\) combinations. "
    obj_dml_data = dml.DoubleMLPanelData(
        data_unbalance_time_type,
        y_col="y",
        d_cols="d",
        t_col="time",
        id_col="id",
    ...
    with pytest.warns(UserWarning, match=msg_warn):
        _ = dml.DoubleMLPLPR(obj_dml_data, ml_l, ml_m, approach="fd_exact", n_folds=2)
