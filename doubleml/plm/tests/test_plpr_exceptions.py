...
dml_data_iv = dml.DoubleMLPanelData(
    plpr_data,
    y_col="y",
    d_cols="d",
    t_col="time",
...
dml_data_binary = dml.DoubleMLPanelData(
    plpr_data_binary,
    y_col="y",
    d_cols="d",
    t_col="time",
...
dml_plpr = dml.DoubleMLPLPR(dml_data, ml_l, ml_m)
dml_plpr_iv_type = dml.DoubleMLPLPR(dml_data, ml_l, ml_m, ml_g, score="IV-type")
...
def test_plpr_exception_data():
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml.DoubleMLPLPR(dml_data_iv, ml_l, ml_m)
...
def test_plpr_exception_scores():
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, ml_l, ml_m, score="IV")
    msg = "score should be a string. 0 was passed."
    with pytest.raises(TypeError, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, ml_l, ml_m, score=0)
...
def test_plpr_exception_approach():
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, ml_l, ml_m, approach="cre")
    msg = "approach should be a string. 4 was passed."
    with pytest.raises(TypeError, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, ml_l, ml_m, approach=4)
...
def test_plpr_exception_smpls():
    ...
    dml_plpr_no_smpls = dml.DoubleMLPLPR(dml_data, ml_l, ml_m, draw_sample_splitting=False)
    with pytest.raises(ValueError, match=msg):
...
def test_plpr_exception_learner():
    ...
    with pytest.raises(TypeError, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, Lasso, ml_m)
    msg = err_msg_prefix + r"BaseEstimator\(\) has no method .fit\(\)."
    with pytest.raises(TypeError, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, BaseEstimator(), ml_m)
    # msg = err_msg_prefix + r'_DummyNoSetParams\(\) has no method .set_params\(\).'
    with pytest.raises(TypeError):
        _ = dml.DoubleMLPLPR(dml_data, _DummyNoSetParams(), ml_m)
    # msg = err_msg_prefix + r'_DummyNoSetParams\(\) has no method .get_params\(\).'
    with pytest.raises(TypeError):
        _ = dml.DoubleMLPLPR(dml_data, _DummyNoGetParams(), ml_m)
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, Lasso(), LogisticRegression())

    msg = r"For score = 'IV-type', learners ml_l and ml_g should be specified. Set ml_g = clone\(ml_l\)."
    with pytest.warns(UserWarning, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, ml_l=Lasso(), ml_m=ml_m, score="IV-type")

    msg = 'A learner ml_g has been provided for score = "partialling out" but will be ignored.'
    with pytest.warns(UserWarning, match=msg):
        _ = dml.DoubleMLPLPR(dml_data, ml_l=Lasso(), ml_m=Lasso(), ml_g=Lasso(), score="partialling out")
    ...
    with pytest.warns(UserWarning, match=msg_warn):
        dml_plpr_hidden_classifier = dml.DoubleMLPLPR(dml_data_binary, Lasso(), log_reg, approach="cre_normal")
    msg = (
...
def test_plpr_exception_and_warning_learner():
    ...
    with pytest.raises(TypeError):
        _ = dml.DoubleMLPLPR(dml_data, _DummyNoClassifier(), Lasso())
