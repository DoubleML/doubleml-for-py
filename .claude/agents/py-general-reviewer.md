...
1. **Identify Changes**: Run `git diff --name-only origin/main...HEAD -- "*.py"` to identify changed `.py` files across the current branch.
...
- **Vectorization**: Using Python loops over arrays where NumPy or Pandas vectorized operations would be substantially faster in practice (same asymptotic complexity, lower overhead).
